...
class OAuth2Client(BaseModel):
    ...
    contacts: Optional[List[StrictStr]] = Field(default=None, description="OAuth 2.0 Client Contact  An array of strings representing ways to contact people responsible for this client, typically email addresses.")
    ...
    post_logout_redirect_uris: Optional[List[StrictStr]] = Field(default=None, description="Allowed Post-Redirect Logout URIs  Array of URLs supplied by the RP to which it MAY request that the End-User's User Agent be redirected using the post_logout_redirect_uri parameter after a logout has been performed.")
    redirect_uris: Optional[List[StrictStr]] = Field(default=None, description="OAuth 2.0 Client Redirect URIs  RedirectURIs is an array of allowed redirect urls for the client.")
    ...
    request_uris: Optional[List[StrictStr]] = Field(default=None, description="OpenID Connect Request URIs  Array of request_uri values that are pre-registered by the RP for use at the OP. Servers MAY cache the contents of the files referenced by these URIs and not retrieve them at the time they are used in a request. OPs can require that request_uri values used be pre-registered with the require_request_uri_registration discovery parameter.")
    response_types: Optional[List[StrictStr]] = Field(default=None, description="OAuth 2.0 Client Response Types  An array of the OAuth 2.0 response type strings that the client can use at the authorization endpoint. Can be one of:  Needed for OpenID Connect Implicit Grant: Returns ID Token to redirect URI: `id_token` Returns Access token redirect URI: `token` Needed for Authorization Code Grant: `code`")
